...
#faltaria hacer que no se junten los bichos

class Enemy:
    def __init__(self,x,y,id=1,speed_walk=6,speed_run=5,gravity=12,jump_power=30,frame_rate_ms=35,move_rate_ms=33,jump_height=140,p_scale=2.9,interval_time_jump=300) -> None:
    ...
    def change_x(self,delta_x,plataform_list=None):
        if self.is_stunned and not self.is_rolling:
            ...
            if plataform_list != None and self.is_next_to_platform(plataform_list) != None:
                x_platform = self.is_next_to_platform(plataform_list)[1]
                if self.is_next_to_platform(plataform_list)[0] ==DIRECTION_R:
                    self.rect.x =x_platform
                    self.collition_rect.x = x_platform + self.rect.width/6
                    self.ground_collition_rect.x = self.rect.x + self.rect.w / 3
                    self.snowball_collition_rect.x = x_platform
                    self.snowball_ground_collition_rect.x = x_platform
                    self.snowball_top_collition_rect.x = x_platform +self.snowball_collition_rect.width / 4
                elif self.is_next_to_platform(plataform_list)[0] ==DIRECTION_L:
                    self.rect.right = x_platform
                    self.collition_rect.right = x_platform - self.rect.width/6
                    self.ground_collition_rect.right = x_platform - 2*self.rect.w / 3
                    self.snowball_collition_rect.right = x_platform
                    self.snowball_ground_collition_rect.right = x_platform
                    self.snowball_top_collition_rect.right= x_platform - self.snowball_collition_rect.width / 4
        ...
        if self.is_stunned:
            ...
            if(self.tiempo_transcurrido_move >= self.move_rate_ms):
                ...
                if(not self.is_on_plataform(plataform_list)):
                    ...
                    if self.is_next_to_platform(plataform_list) != None:
                        if self.is_next_to_platform(plataform_list)[0] ==DIRECTION_R:
                            self.contador = 51
                        elif self.is_next_to_platform(plataform_list)[0] ==DIRECTION_L:
                            self.contador = 0
    ...
    def is_next_to_platform(self,lista_plataformas):
            ...
            for plataforma in lista_plataformas:
                if ((plataforma.rect.collidepoint(self.collition_rect.midleft) or plataforma.rect.collidepoint(self.collition_rect.midright)) and
                    ...
                    if abs(self.collition_rect.left - plataforma.rect.right) <16:
                        retorno = DIRECTION_R,plataforma.rect.right
                    elif abs(self.collition_rect.right - plataforma.rect.left) <16:
                        retorno = DIRECTION_L,plataforma.rect.left
            return retorno
    ...
    def snow_trap(self, lista_proyectiles, delta_ms,sound_list):
        ...
        for proyectil in lista_proyectiles:
                if self.rect.colliderect(proyectil.rect):
                    pygame.mixer.Sound.play(sound_list[ENEMY_HITTED])
                    self.tiempo_last_hit = self.tiempo_transcurrido
                    self.is_stunned  = True
                    if self.times_hitted < 4:
                        if proyectil.is_buffed:
                            self.times_hitted +=1
                        else:
                            self.times_hitted +=0.5
    ...
    def rolling_ball(self,player,delta_ms,plataform_list):
        self.tiempo_transcurrido +=delta_ms
        if player.is_pushing and self.times_hitted ==4 and player.enemy_pushed_id == self.id:
            self.change_x(player.move_x,plataform_list)
    ...
    def kicked_ball(self,delta_ms,platform_list,sound_list):
        ...
        if self.tiempo_transcurrido_pateado < 5000:
            ...
            if self.rect.left ==0:
                ...
                pygame.mixer.Sound.play(sound_list[BOUNCE])
            if self.is_next_to_platform(platform_list) != None:
                if self.is_next_to_platform(platform_list)[0] ==DIRECTION_R:
                    self.direction_kicked = DIRECTION_R
                    pygame.mixer.Sound.play(sound_list[BOUNCE])
                elif self.is_next_to_platform(platform_list)[0] ==DIRECTION_L:
                    self.direction_kicked = DIRECTION_L
                    pygame.mixer.Sound.play(sound_list[BOUNCE])
        ...
        if self.is_dead :
            if not self.is_exploting:
                if self.tiempo_transcurrido - self.death_time <3000:
                    ...
                    Loot(self.rect.x,self.rect.y+30)
    ...
    def update(self,delta_ms,plataform_list, bullet_list, player, enemy_list,sound_list):
        if self.is_dead:
            self.death_state(enemy_list,plataform_list)
        else:    
            if self.got_kicked:
                self.kicked_ball(delta_ms,plataform_list,sound_list)
            else:
                self.do_movement(delta_ms,plataform_list)
        self.rolling_ball(player,delta_ms,plataform_list)  
        self.check_collition(enemy_list,sound_list)
        self.snow_trap(bullet_list, delta_ms,sound_list)
    ...
    def draw(self,screen):
        ...
        if self.is_stunned and not self.is_rolling and not self.is_dead:
            ...
            self.image_snow = self.animation_snow[int(self.times_hitted-1)]
