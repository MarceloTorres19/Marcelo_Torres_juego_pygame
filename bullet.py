...
class Bullet():
    ...
    def __init__(self,x_init,y_init,velocidad,frame_rate_ms,move_rate_ms, direction, reach, frame,is_buffed) -> None:
        ...
        self.bullet_buffed_r = Auxiliar.getSurfaceFromSeparateFiles("my_images/others/proyectiles_2/({0}).png",3,2,flip=False,scale=2.4)
        self.bullet_buffed_l = Auxiliar.getSurfaceFromSeparateFiles("my_images/others/proyectiles_2/({0}).png",3,2,flip=True,scale=2.4)
        self.frame = frame
        self.animation = self.bullet_r 
        self.is_buffed = is_buffed
        if direction == DIRECTION_L:
            self.animation = self.bullet_l
        else:
            x_init +=20
        if self.is_buffed and direction == DIRECTION_L:
            self.animation = self.bullet_buffed_l
        elif self.is_buffed and direction == DIRECTION_R:
            self.animation = self.bullet_buffed_r
            x_init +=20
